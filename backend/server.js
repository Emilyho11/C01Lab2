    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      if (data.deletedCount == 0) {
        return res.status(404).json({
          error: `Note with ID ${noteId} belonging to the user not found`,
        });
      }
...
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
  try {
    ...
    if (!(title || content)) {
      return res
        .status(400)
        .json({ error: "At least title or content are required." });
    }
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      if (title) toChange.title = title;
      if (content) toChange.content = content;
      // Find note with given ID
      const collection = db.collection(COLLECTIONS.notes);
